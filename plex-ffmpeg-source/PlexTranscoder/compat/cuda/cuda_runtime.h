...
typedef struct __device_builtin__ float3
{
    float x, y, z;
} float3;

typedef struct __device_builtin__ ushort3
{
...
#define make_ushort3(a, b, c) ((ushort3){.x = a, .y = b, .z = c})
...
static __inline__ __device__ float clamp(float val, float min, float max)
{
    if (val < min)
        return min;
    else if (val > max)
